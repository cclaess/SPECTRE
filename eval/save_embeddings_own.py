...
def ensure_centers_in_excel(data_dir, excel_path, id_col='PatientID'):
    """
...
# -------------------------------------------------------------------------
# 2. TRANSFORM: Crop using Pre-Calculated Coords
# -------------------------------------------------------------------------
class CropLungsFromCoordsd(MapTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            ...
            # Retrieve center from data dict (populated by Dataset)
            if 'lung_center' in d and d['lung_center'] is not None:
                center = np.array(d['lung_center'])
            else:
                # Fallback to image center if missing/failed (-1)
            ...
            # Handle padding if box is outside image
...
# -------------------------------------------------------------------------
# 3. DATASET (Images + Coords only)
# -------------------------------------------------------------------------
class ImageOnlyNiftiDataset(Dataset):
    def __init__(self, data_dir: str, df: pd.DataFrame, transform=None):
        """
        Accepts the already loaded (and updated) DataFrame.
        """
        image_paths = list(Path(data_dir).glob("*.nii.gz"))
        
        data = []
        for image_path in image_paths:
            p_id = image_path.name.split(".")[0] 
            center = None
            
            # Match with DataFrame (ID assumption)
            id_col = 'patient_id' # CHANGE THIS IF NEEDED
            ...
            data.append({
                ...
                "lung_center": center # Pass center to transform
            })
...
def get_args_parser():
    ...
    parser.add_argument("--save_dir", type=str, default=r"/home/20203686/crop embed")
